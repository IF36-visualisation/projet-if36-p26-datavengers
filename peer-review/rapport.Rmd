...
speedrun <- left_join(speedrun, games_data, by = "gameId")
speedrun <- speedrun %>% rename(platformId = platforms, genreId = genres, releaseDateGame = releaseDate)
speedrun <- left_join(speedrun, platforms_data, by = "platformId")
speedrun <- left_join(speedrun, genres_data, by = "genreId")
speedrun <- speedrun %>%
  mutate(
...
speedrun <- speedrun %>%
  mutate(
    GenreName    = if_else(gameName == "Seterra (Old Version)", "Quiz", GenreName),
    platformName = if_else(gameName == "Seterra (Old Version)", "Web", platformName)
  )
speedrun <- speedrun %>%
  mutate(
    GenreName    = if_else(gameName == "Seterra (Geoguessr Version)", "Quiz", GenreName),
    platformName = if_else(gameName == "Seterra (Geoguessr Version)", "Web", platformName)
  )
```
