...
speedrun0 <- speedrun0 %>%
  mutate(
    verifiedDate = as.Date(verifiedDate)
  ) %>%
  filter(
    !is.na(verifiedDate),
...
speedrun <- speedrun %>% select(gameName,GenreName,platformName,dateJeu)
>>>>>>> Stashed changes
```
...
```
À la lecture du graphique, nous nous rendons compte que la popularité du jeu ne suffit pas pour en faire un jeu très speedrunné, car beaucoup de jeux qu'on ne considère pas comme des « classiques » du jeu vidéo sont présents dans ce classement, notamment Seterra (un jeu de quiz), Celeste (un jeu de plateforme) ou encore ULTRAKILL (un FPS).

Le cas de Seterra est particulièrement révélateur : avec plus de 43 000 runs, soit plus du double du second du classement, ce jeu de quiz géographique gratuit et accessible en ligne se distingue surtout par son format. Des sessions très courtes, une rejouabilité immédiate et des mécaniques répétables en font un terrain idéal pour le speedrun, bien plus qu'un titre AAA classique.

Nous en concluons donc que la popularité ne fait pas tout et qu'un jeu peu connu peut être speedrunné s'il a les bons atouts, notamment un format adapté (durée d'une run, accessibilité, mécaniques répétables). Cependant, au vu de la grande quantité de jeux connus dans le classement, nous aurions plus de chances de produire un jeu speedrunné s'il est très populaire.

Notre objectif est donc de concevoir un jeu populaire avec de bonnes mécaniques de speedrun.
...
Au vu des caractéristiques propres au speedrun (rejouabilité, sessions courtes), nous imaginons que les genres dominants seront les jeux de plateforme et les FPS, au détriment des RPG ou des jeux de stratégie, qui impliquent des parties beaucoup plus longues.
...
runs_par_genre <- speedrun %>%
  ...
  mutate(GenreName = if_else(GenreName == "3D Platformer", "Platformer", GenreName)) %>%
  count(GenreName, name = "nb_runs")
